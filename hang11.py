...
class word(object):
     ...
     def test_isecretwordGuessed(self,letter):
        ...
        if  not self.isWordGuessed(game2):
           game2.setletterguessed("a")
           guessed=''
           print game2.getletterguessed()
           if not  self.isWordGuessed(game2):
             game2.setletterguessed("b")
           print game2.getletterguessed()
           self.isWordGuessed(game2)
           if  self.isWordGuessed(game2):
              print "deu tudo certo no metodo iswordguessed"  
        
     def __init__ (self,secretword): 
         self.secretword=secretword
     def validaletterguessed(self,game):
         if not game.lettersGuessed=="[]":
           return True
     def guessedword(self,game): 
      letter=game.getletter()
      guessed=''
      if self.validaletterguessed(game):   
     ...
     def quantitydifferent(self,game):
...
class game(object):
    ...
    def setletterguessed(self,letter):
        if self.validaletra(letter):  
    ...
    def setletter(self,letter):
        if self.validaletra(self.letter):
           self.letter=letter             
